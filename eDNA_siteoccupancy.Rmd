...
```{r user settings}
##qPCR Replicates
num.reps=6 ##set number of qPCR replicates per sample

##Model Predictors
predictors=c("Lat", "Lon", "Depth_m", "Substrate") ##occupancy modeling habitat covariates (must use exact column name from metadata)
```
...
- **Cq Data** $\Rightarrow$ qPCR data with Cq-values as exported from the BMS MIC Workbench software. Columns must include:
...
```{r import data}
##METADATA
metadata=read.csv(file.choose(), header = T) ##choose metadata file
...
##CQ-VALUES
Cq.data=read.csv(file.choose(), header=T) ##choose Cq data file
...
- **Presence (z)**: proportion of model iterations in which eDNA presence is inferred at a given site, based on the observed data—eDNA qPCR detections, visual survey presence-absence, and habitat covariates—even when neither molecular detection nor visual observation occurred. When the target organism is visually observed, the posterior probability of presence is fixed, but visual absences do not override eDNA-based presence estimates.
- **Occupancy ($\psi$)**: probability that a new site, with the same covariates, is occupied (*i.e.*, the inherent probability of DNA presence based on site characteristics), providing a predictive measure of site suitability when detection errors are considered. eDNA detections and non-detections are incorporated into the model as observed data that inform the posterior probability of presence for a specific site. These detections also influence the occupancy probability and refine the relationship between covariates and occupancy.
- **Sample capture ($\theta$)**: how likely target eDNA is to make it into biological replicates at the field sampling stage
  - **$\theta_{11}$**: probability of a sample from an occupied site containing target eDNA (*i.e.*, true positive capture)
  - **$\theta_{10}$**: capture within a sample from a site that was not occupied (*i.e.*, false-positive sample inference)
...
- **Detection ($p$)**: how likely technical replicates detect target eDNA at the laboratory stage)
  - **$p_{11}$**: probability of a positive qPCR replicate from a sample containing target eDNA (*i.e.*, true positive detection)
...
We therefore focus on the probability of **presence (z)** as an indicator of the underlying reality of nuisance alga eDNA presence and use the **occupancy probability (ψ)** to illustrate the uncertainty associated with imperfect detections across habitat covariates.
...
- Under **Data** tab, assign columns: 
  ...
  - *Column of categorical covariates* $\Rightarrow$ Optional categorical covariate column or 0 if none (7 in the example data)
...
- Under **Settings** tab: 
  ...
  - Set *Number of Independent eDNA qPCR replicates* to "num.reps" in this code (or 6 for the example data)
...
data.psi$Site=NULL
data.psi$Site=occupancy.meta$Site
data.psi=merge(metadata.unique,data.psi, by="Site")

##Sample Capture (theta)
data.theta <- read.csv("RShinyAppData/download_theta11.csv", header=T)
data.theta$Site=NULL
data.theta$Site=occupancy.meta$Site
data.theta=merge(metadata.unique,data.theta, by="Site")

##Detection
data.p <- read.csv("RShinyAppData/download_p11.csv", header=T)
data.p$Site=NULL
data.p$Site=occupancy.meta$Site
data.p=merge(metadata.unique,data.p, by="Site")
```
...
occupancy.siteplot <- ggplot(data.psi, aes(Site, occupancy_Posterior.Mean)) +
  ...
  geom_point(aes(shape=Visual_presence, color=Visual_presence), size = 2, alpha = 1)+
...
```{r}
##Presence (Z)
presenceplot=ggplot(data.psi, aes(Depth_m, presence_Posterior.Mean)) +geom_smooth(method="gam", se=F, na.rm=T, lwd=0.5) + ylim(c(0,1)) +  geom_errorbar(aes(ymin=presence_2.5Credible.Interval, ymax=presence_97.5Credible.Interval), alpha=0.5, width = 0.001) + labs(x="", y="Presence (z)") + geom_point(aes(shape = Visual_presence, color=Visual_presence), size = 2) + 
  theme_bw() + theme(legend.position="none")

##Occupancy (PSI)
psiplot=ggplot(data.psi, aes(Depth_m, occupancy_Posterior.Mean)) + geom_smooth(method="gam", se=F, na.rm=T, lwd=0.5)+ ylim(c(0,1)) + geom_errorbar(aes(ymin=occupancy_probability_2.5Credible.Interval, ymax=occupancy_97.5Credible.Interval), alpha=0.5, width = 0.001) + geom_point(aes(shape = Visual_presence, color=Visual_presence), size = 2) + 
  labs(x="", y="Occupancy (\u03A8)") +
  theme_bw() + theme(legend.position="none")

##Capture (THETA11)
thetaplot=ggplot(data.theta, aes(Depth_m, Posterior.Mean)) + geom_smooth(method="gam", se=F, na.rm=T, lwd=0.5) + ylim(c(0,1)) + geom_errorbar(aes(ymin=X2.5Credible.Interval, ymax=X97.5Credible.Interval), alpha=0.5, width = 0.001) + geom_point(aes(shape = Visual_presence, color=Visual_presence), size = 2) + 
  labs(x="Water Depth (m)", y="Capture (\u03B8)") +
  theme_bw() + theme(legend.position="none")

##Detection (P11)
pplot=ggplot(data.p, aes(Depth_m, Posterior.Mean)) + geom_smooth(method="gam", se=F, na.rm=T, lwd=0.5) + ylim(c(0,1)) + geom_errorbar(aes(ymin=X2.5Credible.Interval, ymax=X97.5Credible.Interval), alpha=0.5, width = 0.001) + geom_point(aes(shape = Visual_presence, color=Visual_presence), size = 2) + 
